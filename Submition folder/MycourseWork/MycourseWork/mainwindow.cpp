...
void MainWindow::updateMetaData()
{
    ...
    if (metaData.value(QMediaMetaData::CoverArtImage).isNull()) {
        ...
        if (coverArtVariant.canConvert<QImage>()) {
            ...
            // Handle the case where the variant cannot be converted to QImage
            QPixmap defaultPixmap("C:/Users/Faiz/Music/Songs/rah.JPG");
            ui->label_2->setPixmap(defaultPixmap);
            ui->label_2->setScaledContents(true);
            qDebug() << "CoverArtImage cannot be converted to QImage";
        }
    } else {
        // Handle the case where the metaData does not contain CoverArtImage
        qDebug() << "MetaData does not contain CoverArtImage";
    }
}
